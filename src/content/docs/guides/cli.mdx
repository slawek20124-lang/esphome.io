...
upload the configuration keeps the new key, since `old_key:` still reaches the device either way. The upload is
protected by the current key, so this is for a planned change: replace a key you believe leaked by serial flash.
...
Home Assistant. A device whose key Home Assistant provisions is rotated from Home Assistant instead.
...
**`-y`** / **`--yes`**
: Do not ask for confirmation when the API key changes too.
