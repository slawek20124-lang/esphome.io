...
- **expect_broadcast_write_response** (*Optional*, boolean): When the controller's `address` is `0` (see
  `allow_broadcast_read` on [Modbus Controller](/components/modbus_controller/)), wait for the reply to each write
  instead of treating it as a fire-and-forget broadcast. Rejected under a controller at any other address.
  Defaults to `false`.
