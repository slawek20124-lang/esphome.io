...
- **address** (*Optional*, int): The Modbus device address (can be decimal or hexadecimal). `1`-`247` is the standard
  device range. `0` is the broadcast address and is **rejected at validation** unless `allow_broadcast_read` is
  set, since a broadcast never returns a response; use the [Modbus Client](/components/modbus_client/) component
  for broadcast or other ad-hoc frames. Values from `248` to
...
- **allow_broadcast_read** (*Optional*, boolean): Allow `address: 0` and poll it as if it were a unicast address, for
  a device that doesn't follow the Modbus spec and replies to the broadcast address. Only accepted together with
  `address: 0`. Every read this controller
  sends waits for its reply with the normal timeout and retry ladder; the device must answer as address `0`.
  Writes from this controller's entities stay fire-and-forget broadcasts unless the entity sets
  `expect_broadcast_write_response`, and a `custom_pdu` whose function code can be broadcast (a write or a vendor
  code) is rejected, since it would never be answered. Defaults to `false`.
