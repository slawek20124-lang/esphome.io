...
The `improv_ble` component in ESPHome implements the open [Improv standard](https://www.improv-wifi.com/)
for configuring Wi-Fi on an ESP32 device by using Bluetooth Low Energy (BLE) to receive the credentials.

The `improv_ble` component will automatically set up the [BLE Server](/components/esp32_ble/).

> [!NOTE]
> The component was renamed from `esp32_improv` to `improv_ble` in ESPHome 2026.10.0 so the name matches the
> [Improv via Serial](/components/improv_serial/) component and no longer implies an ESP32-only feature. The
> legacy `esp32_improv:` key continues to work as a deprecated alias and will be removed in 2027.4.0.
...
The Improv via BLE component provides various [automations](/automations) that can be used to provide feedback during
...
- <APIRef text="improv_ble_component.h" path="improv_ble/improv_ble_component.h" />
